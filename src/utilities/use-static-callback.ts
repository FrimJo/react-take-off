 ...
 */
export function useStaticCallback<T extends () => any>(callback: T): () => ExtractReturn<T>
export function useStaticCallback<T extends (...args: any[]) => any>(
  callback: T,
  args: ExtractArgs<T>
): () => ExtractReturn<T>

export function useStaticCallback(callback: Function, args: any[] = []): Function {
  const previousCallback = useRef<Function>()
  const previousArgs = useRef([])
  const returnedCallback = useRef<Function>()

  if (previousCallback.current !== callback || arraysNotEqual(previousArgs.current, args)) {
 ...
 */
export function useStaticCallbackCreator<T extends () => (...args: any[]) => any>(
  callback: T
): ExtractReturn<T>
export function useStaticCallbackCreator<T extends (...args: any[]) => (...args: any[]) => any>(
  callback: T,
  args: ExtractArgs<T>
): ExtractReturn<T>

export function useStaticCallbackCreator(callbackCreator: Function, args: any[] = []): Function {

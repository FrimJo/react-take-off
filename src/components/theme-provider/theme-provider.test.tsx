...
describe('', () => {
  test('that we get correct theme for Material UI', () => {
    ...
    const testMuiTheme: Theme = JSON.parse(getByTestId('test-mui-theme').textContent || JSON.stringify({}))
    expect(testMuiTheme).toEqual(mockTheme)
  })

  test('that we get correct theme for Emotion', () => {
    const { getByTestId } = renderThemeProvider(<Component />)
    const testEmotionTheme: Theme = JSON.parse(getByTestId('test-e-theme').textContent || JSON.stringify({}))
